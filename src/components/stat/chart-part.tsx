...
export function useChartPart({
  ...
  structureDimension?: "category" | "user";
  ...
  const charts = useMemo(() => {
    ...
    const useCategoryStructure = structureDimension === "category";
    return [
      focusType === "expense"
  ...
  const onStructureChartClick = useCallback((params: ECElementEvent) => {
    if (params.componentType === "series" && params.seriesType === "pie") {
      setSelectedCategoryId((params.data as Series[number]["data"][number]).id);
    }
  }, []);
  ...
  const calendarRange = useMemo(
    ...
    [filtered[0]?.time, filtered[filtered.length - 1]?.time, filtered.length],
  );
  const Part = (
    <>
                ...
                <div className="table-row-group divide-y">
                    ...
                    onItemMoneyClick={(v) => {
                      seeDetails({
                        categories: categories
                          .filter((c) => c.id === v.id || c.parent === v.id)
                          .map((c) => c.id),
                      });
                    }}
                  />
                  {selectedCategoryChart && (
                    <>
                      ...
                      <ListChart
                        series={selectedCategoryChart.series}
                        focusType={focusType}
                        onItemMoneyClick={(v) => {
                          seeDetails({
                            categories: [v.id],
