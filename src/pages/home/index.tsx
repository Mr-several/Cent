...
const STATS_COMMIT_DELAY_MS = 140;
...
export default function Page() {
  ...
  const [isFastBrowsing, setIsFastBrowsing] = useState(false);
  const pendingCommitTimerRef = useRef<number | null>(null);
  const latestPreviewDayRef = useRef(previewDate.format("YYYY-MM-DD"));
  ...
  const sumByType = (list: typeof bills, type: "expense" | "income") =>
    amountToNumber(
      list.filter((b) => b.type === type).reduce((p, c) => p + c.amount, 0),
    );

  const todayBills = useMemo(
    () =>
      filterOrderedBillListByTimeRange(filteredBills, [
        committedDate.startOf("day"),
        committedDate.endOf("day"),
      ]),
    [filteredBills, committedDate],
  );

  const monthBills = useMemo(
    () =>
      filterOrderedBillListByTimeRange(filteredBills, [
        committedDate.startOf("month"),
        committedDate.endOf("month"),
      ]),
    [filteredBills, committedDate],
  );
  ...
  const todayExpense = useMemo(
    () => sumByType(todayBills, "expense"),
    [todayBills],
  );
  // eslint-disable-next-line react-hooks/exhaustive-deps
  const todayIncome = useMemo(
    () => sumByType(todayBills, "income"),
    [todayBills],
  );
  // eslint-disable-next-line react-hooks/exhaustive-deps
  const monthExpense = useMemo(
    () => sumByType(monthBills, "expense"),
    [monthBills],
  );
  // eslint-disable-next-line react-hooks/exhaustive-deps
  const monthIncome = useMemo(
    () => sumByType(monthBills, "income"),
    [monthBills],
  );
  ...
  const onDateClick = useCallback(
    (date: dayjs.Dayjs) => {
      ...
      latestPreviewDayRef.current = dayKey;
      setPreviewDate(date);
      setCommittedDate(date);
      setIsFastBrowsing(false);
  ...
  const onVisibleDatePreview = useCallback((date: dayjs.Dayjs) => {
    const dayKey = date.format("YYYY-MM-DD");
    if (dayKey === latestPreviewDayRef.current) return;
    latestPreviewDayRef.current = dayKey;
    setPreviewDate(date);
    setIsFastBrowsing(true);

    if (pendingCommitTimerRef.current !== null) {
      window.clearTimeout(pendingCommitTimerRef.current);
    }
    pendingCommitTimerRef.current = window.setTimeout(() => {
      setCommittedDate((prev) => (prev.isSame(date, "day") ? prev : date));
      setIsFastBrowsing(false);
      pendingCommitTimerRef.current = null;
    }, STATS_COMMIT_DELAY_MS);
  }, []);
        ...
        <div className="w-full h-full">
          {bills.length > 0 ? (
            <Ledger
              ...
              onVisibleDateChange={onVisibleDatePreview}
